...
class userDetailsController {
    ...
    static registerUser = async (req, res) => {
        ...
        if (number.length !== 10) {
            return res.status(400).send({ message: "Invalid Phone Number", data: null });
        }
    ...
    static loginUser = async (req, res) => {
        ...
        await executeQuery("CALL LoginUser(?)", [number]).then(async (results) => {
            if (results.status === 200) {
                ...
                const passStatus = await bcrypt.compare(password, results.data[0][0].user_pass).then((passRes) => {
                    return passRes;
                });
                if (!passStatus) {
                    return res.status(400).send({ message: "Incorrect credentials!!", data: null });
                }
